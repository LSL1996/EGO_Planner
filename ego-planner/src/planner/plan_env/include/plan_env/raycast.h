...
double signum(double x);//符号函数 

double mod(double value, double modulus);//取余数函数 

double intbound(double s, double ds);//求最小的t，使s+t*ds为整数  
...
public:
  ...
  bool setInput(const Eigen::Vector3d& start,
                ...
                const Eigen::Vector3d& max */);//设置输入 

  bool step(Eigen::Vector3d& ray_pt);//单步更新 
};
