...
#define QUARD_MINIMIZER2_LBFGS(qm, u, du, v, dv) \
    ...
    {
        void *memblock = malloc(size);//分配长度为size字节的内存块 如果分配成功则返回指向被分配内存的指针，否则返回空指针NULL
        //https://blog.csdn.net/xw13106209/article/details/4962479
        if (memblock)//如果分配成功
        {
            memset(memblock, 0, size);//这段空间分配数据为0
        }
        return memblock;
    }
                              ...
                              lbfgs_parameter_t *_param)
    {
        ...
        /* Check the input parameters for errors. *///检查输入是否有错误
        if (n <= 0)
        {
        ...
        /* Allocate working space.初始化工作内存空间 */
